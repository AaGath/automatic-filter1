...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons= [[
            InlineKeyboardButton('🕵 Creator 🕵', url='https://t.me/Wireless_TG'),
    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            for file in files:
                file_id = file.file_id
                btn.append(
                    [InlineKeyboardButton(text=f"{get_size(file.file_size)}", callback_data=f'files_#{file_id}')]
                    )
        else:
...
<i>Please Go to Google and Confirm the Correct Spelling 🥺🙏</i></b>""",
        ...
        if imdb and imdb.get('poster'):
            ...
            await message.reply_text(f"<b>🔍 Here is Results For Your Request {search} ‌‌‌‌‎ 🔍</b>", reply_markup=InlineKeyboardMarkup(btn))
